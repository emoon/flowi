...
static void fl_window_end(struct FlWindowApi* api);

// Call between begin() and end() to create a child window. Child windows can embed their own child.
static bool fl_window_begin_child(struct FlWindowApi* api, const char* id, FlVec2 size, bool border,
                                  FlWindowFlags flags);

// End call for various types such as windows, lists, etc.
static void fl_window_end_child(struct FlWindowApi* api);

// Returns true if the window is appearing after being hidden/inactive (or the first time)
static bool fl_window_is_appearing(struct FlWindowApi* api);

// Is current window collpased?
static bool fl_window_is_collapsed(struct FlWindowApi* api);

// is current window focused? or its root/child, depending on flags. see flags for options.
static bool fl_window_is_focused(struct FlWindowApi* api, FlFocusedFlags flags);
...
static bool fl_window_is_hovered(struct FlWindowApi* api, FlHoveredFlags flags);

// get dpi scale currently associated to the current window's viewport.
static float fl_window_dpi_scale(struct FlWindowApi* api);

// get current window position in screen space (useful if you want to do your own drawing via the drawlist api)
static FlVec2 fl_window_pos(struct FlWindowApi* api);

// get current window size
static FlVec2 fl_window_size(struct FlWindowApi* api);
