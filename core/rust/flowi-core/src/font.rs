...
impl Font {
    /// Create a font from (TTF) file. To use the font use [Font::set] or [Font::set_with_size] before using text-based widgetsReturns >= 0 for valid handle, use fl_get_status(); for more detailed error message
    ...
    pub fn set(font: Font) {
        unsafe {
            fl_font_set_impl(font);
        }
    }

    /// Set font active with specific size in pixels
    pub fn set_with_size(size: u32) {
        unsafe {
            fl_font_set_with_size_impl(size);
        }
    }
